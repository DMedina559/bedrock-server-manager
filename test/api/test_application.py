...
class TestApplicationInfo:
    def test_get_application_info_api(self, real_manager):
        with patch(
...
class TestContentListing:
    def test_list_available_worlds_api(self, real_manager, tmp_path):
        ...
        with patch(
            ...
            with patch.object(
                real_manager,
                "_list_content_files",
                return_value=[str(worlds_dir / "world1.mcworld")],
            ):
    ...
    def test_list_available_addons_api(self, real_manager, tmp_path):
        ...
        with patch(
            ...
            with patch.object(
                real_manager,
                "_list_content_files",
                return_value=[str(addons_dir / "addon1.mcpack")],
            ):
...
class TestGetAllServersData:
    def test_get_all_servers_data_success(self, real_manager):
        with patch.object(
            real_manager, "get_servers_data", return_value=([{"name": "server1"}], [])
        ):
            with patch(
                "bedrock_server_manager.api.application.get_manager_instance",
                return_value=real_manager,
