...
async def test_add_players_manually_api_success(app_context, monkeypatch):
    ...
    result = await add_players_manually_api(["user1:1234", "user2:5678"], app_context)
...
async def test_add_players_manually_api_invalid_input(app_context):
    """Test add_players_manually_api catches bad inputs directly."""
    result = await add_players_manually_api([], app_context)
    assert result["status"] == "error"
    assert "non-empty list" in result["message"]


async def test_add_players_manually_api_parse_error(app_context, monkeypatch):
    ...
    result = await add_players_manually_api(["bad_format"], app_context)
...
async def test_get_all_known_players_api_success(app_context, monkeypatch):
    """Test get_all_known_players_api loads generic players array via mock."""
    mock_get = AsyncMock(return_value=[{"name": "p1"}, {"name": "p2"}])
...
async def test_scan_and_update_player_db_api_success(app_context, monkeypatch):
...
async def test_scan_and_update_player_db_api_bsm_error(app_context, monkeypatch):
