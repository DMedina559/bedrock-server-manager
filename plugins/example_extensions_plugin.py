...
def hello_cli_command(ctx, name):
    ...
    click.echo(f"Hello, {name}, from the Example Extensions Plugin's CLI command! (Plugin v{VERSION})")

@example_plugin_cli_group.command("info")
def info_cli_command():
    """Shows info about this example plugin CLI extension."""
    click.echo("This is the 'info' command of the 'explug' group from ExampleExtensionsPlugin.")
    click.echo(f"Plugin Version: {VERSION}")
    if not HAS_AUTH_DEPENDENCY:
        click.echo("Note: FastAPI auth dependency 'get_current_user' was not found during plugin load.")
...
async def get_plugin_api_status():
    ...
    return {"plugin_name": "Example Extensions Plugin", "version": VERSION, "api_status": "operational"}
...
async def greet_name_via_api(name: str):
    ...
    return {"message": f"Hello, {name}, from the Example Extensions Plugin's API! (Plugin v{VERSION})"}
...
class ExampleExtensionsPlugin(PluginBase):
    ...
    def on_load(self):
        self.logger.info(f"Example Extensions Plugin v{self.version} loaded successfully.")
        if not HAS_AUTH_DEPENDENCY:
            self.logger.warning("FastAPI auth dependency 'get_current_active_user' not found. API endpoints might be unsecured if this is unintentional.")
        self.logger.info("This plugin demonstrates adding a CLI command group 'explug' with subcommands, and a FastAPI router at '/example_plugin_api'.")

    def get_cli_commands(self):
        self.logger.info(f"ExampleExtensionsPlugin v{self.version}: Providing CLI command group 'explug'.")
        # Return the top-level group. Click will handle its subcommands.
        return [example_plugin_cli_group]

    def get_fastapi_routers(self):
        self.logger.info(f"ExampleExtensionsPlugin v{self.version}: Providing FastAPI router for '/example_plugin_api'.")
