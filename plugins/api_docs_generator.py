...
class APIDocsGenerator(PluginBase):
    ...
    def _format_as_markdown(self, api_list: list) -> str:
        ...
        # --- Document Header ---
        lines = [
            f"## Plugin API Reference",
            f"> _This document was auto-generated by the `api_docs_generator` plugin on {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}._",
            f"> _Application Version: {__version__}_",
            "\n---",
            "This document lists all functions available to plugins via the `self.api` object.",
