...
const ComponentRegistry = {
    // Layout
    Container: ({ children, className = "" }) => <div className={`container ${className}`}>{children}</div>,
    Card: ({ title, children, className = "" }) => (
        <div className={`server-card ${className}`}>
            {title && <div className="server-card-header"><h3>{title}</h3></div>}
            <div className="server-card-body">{children}</div>
        </div>
    ),
    Row: ({ children, gap = "10px", className = "" }) => (
        <div className={className} style={{ display: 'flex', flexDirection: 'row', gap, flexWrap: 'wrap' }}>{children}</div>
    ),
    Column: ({ children, gap = "10px", className = "", flex = 1 }) => (
        <div className={className} style={{ display: 'flex', flexDirection: 'column', gap, flex }}>{children}</div>
    ),
    ...
    Label: ({ content, htmlFor, className = "" }) => <label htmlFor={htmlFor} className={`form-label ${className}`}>{content}</label>,

    // Basic Inputs
    Button: ({ label, onClick, variant = "primary", icon, disabled = false, className = "" }) => {
    ...
    Input: ({ type = "text", value, onChange, placeholder, className = "", readOnly = false }) => (
        <input
            type={type}
            value={value}
    ...
    Icon: ({ name, size = 20, className = "" }) => {
        const icons = {
            Activity, AlertCircle, CheckCircle2, Info, Terminal, Save, Trash2, Plus, X, Upload
        };
...
const DynamicPage = () => {
    ...
    const renderNode = (node, key) => {
        ...
        if (!Component) {
            ...
            return <div key={key} style={{ color: 'red', border: '1px dashed red', padding: '5px' }}>Unknown component: {node.type}</div>;
        }
        ...
        if (node.type === 'Input') {
            if (props.id) {
                ...
                props.value = stateValue !== undefined ? stateValue : (props.value || props.defaultValue || "");
    ...
    if (error) return <div className="container"><div className="message error">Error: {error}</div></div>;
    ...
    return (
        <div className="dynamic-page-wrapper">
             ...
             {Array.isArray(schema) ? schema.map((node, i) => renderNode(node, i)) : renderNode(schema, 0)}
        </div>
    );
};
