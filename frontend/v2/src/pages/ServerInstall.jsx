...
const ServerInstall = () => {
        ...
        try {
            ...
            if (response && response.status === "confirm_needed") {
                if (confirm(response.message)) {
                    ...
                    const confirmResponse = await post("/api/server/install", confirmData);
    ...
    return (
        <div className="container">
            ...
            <div style={{ maxWidth: "600px", margin: "0 auto", background: "var(--container-background-color)", padding: "30px", border: "1px solid var(--border-color)", borderRadius: "8px" }}>
                <form onSubmit={handleSubmit} className="form-group">

                    <div style={{ marginBottom: "20px" }}>
                        <label className="form-label" htmlFor="server_name">Server Name</label>
                        <input
                            type="text"
                            id="server_name"
                            name="server_name"
                            ...
                            disabled={loading}
                        />
                        <small style={{ color: "#888" }}>Unique name for the server instance.</small>
                    </div>

                    <div style={{ marginBottom: "20px" }}>
                        <label className="form-label" htmlFor="server_version">Server Version</label>
                        <select
                            id="server_version"
                            name="server_version"
                            className="form-input"
                        ...
                        >
                            ...
                            <option value="SPECIFIC">SPECIFIC VERSION (Enter manually)</option>
                        </select>
                    </div>

                    {formData.server_version === "SPECIFIC" && (
                        <div style={{ marginBottom: "20px", padding: "15px", background: "#f9f9f9", border: "1px solid #ddd", borderRadius: "5px" }}>
                            <label className="form-label" htmlFor="specificVersion">Enter Version Number</label>
                            <input
                                type="text"
                                id="specificVersion"
                                className="form-input"
                    ...
                    {formData.server_version === "CUSTOM" && (
                        <div style={{ marginBottom: "20px", padding: "15px", background: "#f9f9f9", border: "1px solid #ddd", borderRadius: "5px" }}>
                            <label className="form-label" htmlFor="server_zip_path">Select Custom ZIP</label>
                            {customZips.length > 0 ? (
                                <select
                                    id="server_zip_path"
                                ...
                                <div style={{ color: "red" }}>
                                    No custom ZIP files found in <code>downloads/custom/</code>. Please upload one first.
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <button type="submit" className="action-button" disabled={loading} style={{ width: "100%", justifyContent: "center" }}>
                        {loading ? <RefreshCw className="spin" size={20} style={{ marginRight: "8px" }} /> : <PlusSquare size={20} style={{ marginRight: "8px" }} />}
