...
function _handleInstallSuccessNavigation(apiResponseData) {
  ...
  if (nextUrl) {
    ...
    showStatusMessage(
      "Server installed, but next step URL is missing.",
      "warning",
    );
  }
}
...
function monitorTaskWithWebSocket(taskId, successCallback) {
  ...
  const handleTaskUpdate = (event) => {
    ...
    if (taskData.status === "success") {
      showStatusMessage(
        taskData.message || "Task completed successfully.",
        "success",
      );
...
export async function triggerInstallServer(buttonElement) {
  ...
  if (!serverName)
    return showStatusMessage("Server name cannot be empty.", "warning");
  if (!/^[a-zA-Z0-9_-]+$/.test(serverName))
    return showStatusMessage("Server name has invalid characters.", "error");
  ...
  if (serverVersion.toUpperCase() === "CUSTOM") {
    requestBody.server_zip_path =
      document.getElementById("custom-zip-path").value;
  }

  const startMonitoring = (taskResponse) => {
    if (webSocketClient.shouldUseFallback()) {
      console.warn(
        "WebSocket not available, falling back to polling for task monitoring.",
      );
      pollTaskStatus(taskResponse.task_id, _handleInstallSuccessNavigation);
    } else {
      monitorTaskWithWebSocket(
        taskResponse.task_id,
        _handleInstallSuccessNavigation,
      );
    }
  };
...
async function loadPermissions() {
  ...
  try {
    ...
    if (
      ...
      const noPlayersTemplate = document.getElementById(
        "no-players-row-template",
      );
    ...
    showStatusMessage(
      `Client-side error loading permissions: ${error.message}`,
      "error",
    );
  } finally {
...
export async function savePermissions(buttonElement, serverName, isNewInstall) {
  const permissions = Array.from(
    document.querySelectorAll("select.permission-select"),
  ).map((select) => ({
...
export function initializeInstallConfigPage() {
  ...
  if (serverVersionInput) {
    serverVersionInput.addEventListener("input", (e) =>
      checkCustomVersion(e.target.value),
    );
  ...
  document
    ...
    ?.addEventListener("click", (e) =>
      savePermissions(e.currentTarget, serverName, isNewInstall),
    );
  document
    .getElementById("submit-properties-btn")
    ?.addEventListener("click", (e) =>
      saveProperties(e.currentTarget, serverName, isNewInstall),
    );
  document
    .getElementById("save-service-settings-btn")
    ?.addEventListener("click", (e) =>
      saveServiceSettings(e.currentTarget, serverName, os, isNewInstall),
    );
  ...
  document.querySelectorAll(".toggle-input").forEach((input) => {
    const hidden = input
      ...
      ?.querySelector(
        `.toggle-hidden-false[name="${input.name.replace("-cb", "")}"]`,
      );
    if (hidden) {
  ...
  const setSegmentedControlState = (hiddenInputId, value) => {
    ...
    const controlWrapper = document.querySelector(
      `[data-input-id="${hiddenInputId}"]`,
    );
    if (hiddenInput && controlWrapper) {
      ...
      controlWrapper
        .querySelectorAll(".segment")
        .forEach((s) => s.classList.remove("active"));
      const activeSegment = controlWrapper.querySelector(
  ...
  function populateForm(properties) {
    const propertyMap = {
      ...
      "enable-lan-visibility": {
        id: "enable-lan-visibility-cb",
        type: "toggle",
      },
      "allow-list": { id: "allow-list-cb", type: "toggle" },
      "default-player-permission-level": {
        id: "default-player-permission-level-hidden",
        type: "segment",
      },
  ...
  async function loadProperties() {
    ...
    try {
      ...
      if (data && data.status === "success" && data.properties) {
        ...
        showStatusMessage(
          data?.message || "Failed to load server properties.",
          "error",
        );
      }
    } catch (error) {
      showStatusMessage(
        `Client-side error loading properties: ${error.message}`,
        "error",
      );
    } finally {
      if (loader) loader.style.display = "none";
