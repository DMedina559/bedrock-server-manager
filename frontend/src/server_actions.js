...
function getSelectedServer() {
  ...
  console.debug(
    `${functionName}: Attempting to get selected server from #server-select dropdown.`,
  );

  const serverSelect = document.getElementById("server-select");
  if (!serverSelect) {
    const errorMsg =
      "Internal page error: Server selection dropdown element ('#server-select') not found.";
  ...
  console.debug(
    `${functionName}: Returning selected server name: '${selectedServer}'`,
  );
...
export function startServer(buttonElement) {
  ...
  if (serverName) {
    ...
    const method = "POST";
    console.log(
      `${functionName}: Attempting to start server '${serverName}' via API call to ${actionPath}...`,
    );
    ...
    console.log(
      `${functionName}: API request initiated for '${serverName}' (asynchronous).`,
    );
  } else {
    console.warn(
      `${functionName}: Action aborted because no server was selected.`,
    );
...
export function stopServer(buttonElement) {
  ...
  if (serverName) {
    ...
    const method = "POST";
    console.log(
      `${functionName}: Attempting to stop server '${serverName}' via API call to ${actionPath}...`,
    );
    sendServerActionRequest(
      serverName,
      actionPath,
...
export function restartServer(buttonElement) {
  ...
  if (serverName) {
    ...
    const method = "POST";
    console.log(
      `${functionName}: Attempting to restart server '${serverName}' via API call to ${actionPath}...`,
    );
    sendServerActionRequest(
      serverName,
      actionPath,
...
export function promptCommand(buttonElement) {
  ...
  console.debug(
    `${functionName}: Prompting user for command input for server '${serverName}'.`,
  );
  ...
  console.debug(
    `${functionName}: User entered command: "${command}", Trimmed: "${trimmedCommand}"`,
  );
...
export function deleteServer(buttonElement, serverName) {
  ...
  if (!serverName || typeof serverName !== "string" || !serverName.trim()) {
    const errorMsg =
      "Internal error: Server name is missing or invalid for delete action.";
    console.error(`${functionName}: ${errorMsg}`);
    showStatusMessage(errorMsg, "error");
    return;
  }
  ...
  // --- Confirmation ---
  console.debug(
    `${functionName}: Prompting user for delete confirmation for server '${trimmedServerName}'.`,
  );
  ...
  if (confirm(confirmationMessage)) {
    console.log(
      `${functionName}: Deletion confirmed by user for '${trimmedServerName}'. Sending API request...`,
    );
    // --- Prepare and Send API Request ---
    ...
    console.log(
      `${functionName}: API request initiated for '${trimmedServerName}' (asynchronous).`,
    );
    ...
    console.log(
      `${functionName}: Deletion cancelled by user for '${trimmedServerName}'.`,
    );
...
export function triggerServerUpdate(buttonElement, serverName) {
  ...
  if (!serverName || typeof serverName !== "string" || !serverName.trim()) {
    const errorMsg =
      "Internal error: Server name is missing or invalid for update action.";
    console.error(`${functionName}: ${errorMsg}`);
    showStatusMessage(errorMsg, "error");
    return;
  }
  ...
  const method = "POST";
  console.log(
    `${functionName}: Attempting to update server '${trimmedServerName}' via API call to ${actionPath}...`,
  );
  sendServerActionRequest(
    trimmedServerName,
    actionPath,
